...
int main()
{
  camera_t* cameraLeft = camera_open("/dev/video2", 320, 240);
  camera_t* cameraRight = camera_open("/dev/video1", 320, 240);
  printf("%d",cameraLeft->fd);
  printf("%d",cameraRight->fd);
  camera_init(cameraLeft);
  camera_init(cameraRight);

  camera_start(cameraLeft);
  camera_start(cameraRight);
...
//    camera_frame(cameraRight, timeout);
  ...
  unsigned char* rgbRight =
    yuyv2rgb(cameraRight->head.start, cameraRight->width, cameraRight->height);
  FILE* outRight = fopen("resultRight.jpg", "w");
  ...
  camera_finish(cameraLeft);
  camera_finish(cameraRight);

  camera_close(cameraLeft);
  camera_close(cameraRight);
  return 0;
}
